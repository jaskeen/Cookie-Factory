...
function SpriteFactory:addSpriteSheetModule(...)
    for i, setName in ipairs(arg) do
        ...
        if setNameReferences[setName] == nil then
            setNameReferences[setName] = 1
        else
            setNameReferences[setName] = setNameReferences[setName] + 1
        end
        ...
        if display.contentScaleX ~= 1 then
            ...
            if #scales > 1 then
                for i = 2, #scales do
                    --atrace(scales[i] .. ' ' .. ' ' .. display.contentScaleX)
                    if 1/scales[i] >= display.contentScaleX then
                        sdx = i
                    end
                end
            end

            if scales[sdx] == 1 then
                suffix = ''
            else
                suffix = "@" .. scales[sdx]
...
function SpriteFactory:dispose()
    timer.performWithDelay(1, function()
        for i, setName in pairs(self.setNames) do
            if setNameReferences[setName] then
                setNameReferences[setName] = setNameReferences[setName] - 1
                if setNameReferences[setName] == 0 then
                    require(setName).destroy()
                    unrequire(setName)
                    setNameReferences[setName] = nil
                    --atrace("disposing of " .. setName)
                end
            else
                assert('disposing of non referenced sprite sheet ' .. setName)
            end
        end
...
local decorateGSprite = function (self)
    ...
    self.frameRate = SpriteGroup.frameRate
    self.numFrames = SpriteGroup.numFrames
...
--[[
    ...
    Parameters:
        ...
        - _xOffset Optional x offset for the physics body.
        - _yOffset Optional y offset for the physics body.
        - _scale Optional scale for the physics body.
...
function SpriteFactory:addPhysics(_displayObj, _physics, _bodyType, _properties, _shapeName, _xOffset, _yOffset, _scale)
    ...
    local shapes = self:getShapes(_shapeName, _xOffset, _yOffset, _scale)
    for i = 1, #shapes do
        local props = {}
...
function SpriteFactory:getRectShape(_sequence, _frame, _xOffset, _yOffset, _scale)
    ...
    px = px - (sInfo.unscaledSpriteRect.width * _scale) * 0.5 + _xOffset
    py = py - (sInfo.unscaledSpriteRect.height * _scale) * 0.5 + _yOffset

    pw = sInfo.unscaledSpriteRect.width * _scale
    ph = sInfo.unscaledSpriteRect.height * _scale
...
function SpriteFactory:getShapes(_shapeName, _xOffset, _yOffset, _scale)
    ...
    if (self.spriteShapes[shapeName][1] ~= nil) then
        if _scale == nil or _scale == 1 then
            return (self.spriteShapes[shapeName][1]).polys
        else
            
            local polys =  (self.spriteShapes[shapeName][1]).polys
            local scaledPolys = {}
            for i = 1, #polys do
                local p = polys[i]
                local sp = {}
                ...
                scaledPolys[#scaledPolys + 1] = sp
            end

            return scaledPolys
        end
    else
        return {self:getRectShape(shapeName, 1, _xOffset, _yOffset, _scale)};
    end
end
...
--[[
    ...
    Returns:
        The frameRate of the current sprite sequence in frames per second
]]
function SpriteGroup:frameRate()
    return self.curSprite.sInfo.frameRate
end
...
--[[
    ...
    Returns:
        The number of frames in the current sprite sequence
]]
function SpriteGroup:numFrames()
    return self.curSprite.sInfo.frameCount
end
...
function SpriteGroup:addPhysics(_physics, _bodyType, _properties, _shapeName, _scale)
    ...
    local shapes = self:getShapes(_shapeName, _scale)
    for i = 1, #shapes do
        local props = {}
        for k, v in pairs(_properties) do
            props[k] = v
...
--[[
    ...
    Parameters:
        - _scale Optional scale for the rectangle
...
function SpriteGroup:getRectShape(_scale)
    ...
    px = px - sInfo.unscaledSpriteRect.width * _scale * 0.5
    py = py - sInfo.unscaledSpriteRect.height * _scale * 0.5
...
function SpriteGroup:getShapes(_shapeName, _scale)
    ...
    if (self.spriteFactory.spriteShapes[shapeName][1] ~= nil) then
        if _scale == nil or _scale == 1 then
            ...
            local polys =  (self.spriteFactory.spriteShapes[shapeName][1]).polys
            local scaledPolys = {}
            for i = 1, #polys do
                local p = polys[i]
                local sp = {}
        ...
        return {self:getRectShape(_scale)};
    end
end
